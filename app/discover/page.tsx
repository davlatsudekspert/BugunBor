...
import { RADIUS_PRESETS_KM } from '@/modules/geo/distance';
import { listActiveDeals } from '@/modules/catalog/repository';

export const metadata: Metadata = {
  title: 'Faol aksiyalarni topish',
  description:
    'Shahar, biznes yoki taklif bo‘yicha faol BugunBor aksiyalarini qidiring.',
...
export default async function DiscoverPage({
  ...
  const radiusKm = params.radiusKm ? Number(params.radiusKm) : 5;
  const near =
    lat !== undefined &&
    lng !== undefined &&
    Number.isFinite(lat) &&
      ...
      <section className="border-b border-slate-200 bg-[#f8f1e8]">
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          ...
          <form className="mt-6 flex max-w-3xl flex-col gap-2 rounded-2xl border border-slate-200 bg-white p-2 shadow-sm sm:flex-row">
            <label className="flex h-12 flex-1 items-center gap-2 px-3">
              ...
              <input
                ...
                name="q"
                placeholder="Taom, xizmat yoki biznes…"
                className="min-w-0 flex-1 outline-none"
              />
            </label>
            <select
              name="city"
              defaultValue={params.city ?? 'tashkent'}
            ...
            >
              ...
              <option value="service">✂️ Xizmat</option>
            </select>
            <button
              type="submit"
              className="h-12 rounded-xl bg-primary px-6 text-sm font-bold text-white"
            >
              Topish
            </button>
          </form>
          ...
          <div className="mt-4 flex items-center gap-2 text-sm text-slate-500">
            <ListFilter className="size-4" /> {deals.length} ta mos faol taklif{' '}
            {near ? `— joylashuvingizdan ${radiusKm} km ichida` : `— ${city}`}
          </div>
        </div>
      </section>
        ...
        {deals.length ? (
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {deals.map((deal) => (
              <article
                key={deal.id}
                className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_8px_30px_rgba(20,40,55,.06)]"
              >
                <div className="flex h-28 items-center justify-between bg-[#152a3b] p-5 text-white">
                  <Badge className="bg-primary text-base font-black text-white">
                    -{deal.discountPercent}%
                  </Badge>
                  <span className="text-6xl">
                    {deal.dealType === 'SERVICE'
                      ? '✂️'
                      : deal.categorySlug === 'xaridlar'
                        ? '📚'
                        : '🍽️'}
                  </span>
                </div>
                <div className="p-5">
                  ...
                  <p className="mt-4 text-2xl font-black text-primary">
                    ...
                    <span className="text-sm font-normal text-slate-400 line-through">
                      {formatPrice(deal.originalPriceUzs)}
                    </span>
                  </p>
                  ...
                  <a
                    ...
                    className={cn(
                      buttonVariants(),
                      'mt-5 h-10 w-full rounded-xl font-bold',
                    )}
                  >
                    Batafsil <ArrowRight className="ml-1 size-4" />
                  </a>
                </div>
              </article>
            ))}
          ...
          <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-12 text-center">
            ...
            <h2 className="mt-4 text-xl font-bold">
              Hozircha mos aksiya topilmadi
            </h2>
            <p className="mt-2 text-slate-500">
              Qidiruvni qisqartiring yoki radiusni kengaytiring.
            </p>
          </div>
        )}
      </section>
