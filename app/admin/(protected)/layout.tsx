import { BadgeCheck, LayoutDashboard, Megaphone, MessageCircle, ShieldCheck, ShoppingBag, TicketPercent, Users, Wallet } from 'lucide-react';
...
const navItems: Array<{ href: string; label: string; icon: typeof LayoutDashboard; visible: (role: AdminRole) => boolean }> = [
  ...
  { href: '/admin/promo-codes', label: 'Promokodlar', icon: TicketPercent, visible: (role) => canAdmin(role, 'admin.promocodes.manage') },
  { href: '/admin/announcements', label: 'Reklama va e’lonlar', icon: Megaphone, visible: (role) => canAdmin(role, 'admin.announcements.manage') },
  { href: '/admin/support', label: 'Murojaatlar', icon: MessageCircle, visible: (role) => canAdmin(role, 'admin.support.manage') },
