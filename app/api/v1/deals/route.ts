...
const querySchema = z.object({ region: z.string().trim().max(60).optional(), city: z.string().trim().max(80).optional(), q: z.string().trim().max(120).optional(), limit: z.coerce.number().int().min(1).max(50).optional() });
...
export async function GET(request: Request) {
  ...
  const results = await listActiveDeals({ region: parsed.data.region, city: parsed.data.city, query: parsed.data.q, limit: parsed.data.limit });
