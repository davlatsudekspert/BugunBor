...
import { SESSION_COOKIE, verifyLoginCode } from '@/modules/auth/otp';
...
export async function POST(request: Request) {
  ...
  const result = await verifyLoginCode(phone, parsed.data.code, {
  ...
  const response = NextResponse.json({ data: { displayName: result.user.displayName, role: result.user.role } });
