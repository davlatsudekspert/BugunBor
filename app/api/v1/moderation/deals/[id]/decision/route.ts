...
export async function POST(
  ...
  if (
    ...
    return NextResponse.json(
      { error: { message: 'Moderator ruxsati talab qilinadi.' } },
      { status: 403 },
    );
  ...
  if (!parsed.success)
    return NextResponse.json(
      { error: { message: 'Qaror va kamida 10 belgili sabab kerak.' } },
      { status: 422 },
    );
  ...
  const current = await db
    .prepare(
      'SELECT id, status, starts_at AS "startsAt" FROM deals WHERE id = ?1',
    )
    .bind(id)
    .first<{ id: string; status: string; startsAt: string }>();
  if (!current)
    return NextResponse.json(
      { error: { message: 'Aksiya topilmadi.' } },
      { status: 404 },
    );
  if (current.status !== 'PENDING_REVIEW')
    return NextResponse.json(
      { error: { message: 'Faqat tekshiruvdagi aksiyaga qaror beriladi.' } },
      { status: 409 },
    );
  const nextStatus =
    parsed.data.decision === 'APPROVE'
      ? new Date(`${current.startsAt}Z`) > new Date()
        ? 'SCHEDULED'
        : 'ACTIVE'
      : 'REJECTED';
  ...
  await db.batch([
    db
      .prepare(
        'UPDATE deals SET status = ?1, updated_at = CURRENT_TIMESTAMP WHERE id = ?2 AND status = ?3',
      )
