...
- [x] Customer/business login (`/login`, `modules/auth/otp.ts`) is a real phone + Telegram OTP flow: hashed codes and session tokens (never stored or logged in the clear), per-phone rate limiting (max 3 codes / 10 min) and attempt lockout (max 5 guesses), a `__Host-` prefixed httpOnly session cookie, and a self-service Telegram-linking step (a one-time hashed nonce, `phone_link_tokens`) for a phone with no chat linked yet — the Bot API can't message a chat that hasn't messaged it first. `POST /api/v1/telegram/bot/webhook` (the only inbound path that performs that link) is itself gated by a per-deploy secret (`TELEGRAM_WEBHOOK_SECRET`, generated fresh and registered with Telegram's `setWebhook` on every deploy — see `deploy-cloudflare.yml`) compared with `timingSafeEqual`, and rejects one Telegram account linking to a second phone number.
