...
| `/login` | Phone + Telegram OTP sign-in (implemented — see `README.md` → "Mijoz/biznes uchun kirish"). A first-time phone is walked through linking a Telegram chat by sharing their phone number with the bot (a native "share phone number" button) before its first code can be sent; no separate `/verify` page — code entry is a step within `/login` itself. |
...
- `POST /api/v1/auth/otp/request` (returns `NEEDS_TELEGRAM_LINK` + the bot's link for a phone with no Telegram chat linked yet, otherwise sends a code and returns `SENT`), `POST /api/v1/auth/otp/verify` (sets the `__Host-bugunbor_session` cookie), `POST /api/v1/auth/logout`
- `POST /api/v1/telegram/bot/webhook` (Telegram calls this on every message to the bot; on `/start` it offers a native "share phone number" button, and on the resulting `contact` message it pairs that chat with the shared phone number — see `POST /api/v1/auth/otp/request` above)
