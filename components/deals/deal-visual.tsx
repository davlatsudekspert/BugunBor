...
export function DealVisual({ visual, categorySlug, photo, priority, className, emojiClassName, children }: {
  visual: string | null;
  categorySlug: string;
  photo?: string | null;
  ...
  return (
    <div className={cn('relative overflow-hidden bg-gradient-to-br', gradient, className)}>
      {photo ? (
        <>
          <img src={photo} alt="" loading={priority ? 'eager' : 'lazy'} decoding="async" className="pointer-events-none absolute inset-0 size-full object-cover transition-transform duration-500 group-hover:scale-105" />
