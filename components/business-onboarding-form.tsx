...
export function BusinessOnboardingForm() {
  ...
  async function submit(formData: FormData) {
    ...
    if (!response.ok) {
      setState('error');
      setMessage(result.error?.message ?? 'Ariza yuborilmadi.');
      return;
    }
    setState('success');
  ...
  if (state === 'success')
    return (
      <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-8 text-center">
        ...
        >
          Dashboardga o‘tish <ArrowRight className="size-4" />
        </a>
      </div>
    );
  ...
  return (
    <form
      action={submit}
      className="space-y-5 rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_18px_60px_rgba(20,40,55,.08)] sm:p-8"
    >
      <div className="grid gap-5 sm:grid-cols-2">
        <label className="sm:col-span-2">
          <span className="mb-2 block text-sm font-bold">Biznes nomi</span>
          <input
            required
            name="name"
            minLength={2}
        ...
        <label>
          ...
          >
            ...
            <option value="cat_service">Xizmatlar</option>
          </select>
        </label>
        <label>
          <span className="mb-2 block text-sm font-bold">Shahar</span>
          <select
            required
            name="city"
            className="h-12 w-full rounded-xl border border-slate-200 bg-white px-4"
        ...
        <label className="sm:col-span-2">
          ...
          <textarea
            ...
            maxLength={1200}
            rows={4}
            placeholder="Mijozlarga biznesingiz haqida tabiiy va aniq ayting…"
      ...
      <p className="text-center text-xs leading-5 text-slate-500">
        Yuborish orqali ma’lumotlar tekshirilishiga rozilik bildirasiz. Profil
        tasdiqlanmaguncha ochiq e’lon qilinmaydi.
      </p>
    </form>
  );
}
